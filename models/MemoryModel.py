...
class MemoryModel(torch.nn.Module):
    ...
    def __init__(self, node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        if self.model_name == 'TGN':
            ...
            self.memory_updater = RNNMemoryUpdater(memory_bank=self.memory_bank, message_dim=self.message_dim,
                                                   memory_dim=self.memory_dim)
        else:
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
        ...
        if self.model_name == 'JODIE':
            ...
            src_node_time_intervals = torch.from_numpy(node_interact_times).float().to(self.device) - \
            ...
            # Tensor, shape (batch_size, )
            dst_node_time_intervals = torch.from_numpy(node_interact_times).float().to(self.device) - \
            ...
            node_embeddings = self.embedding_module.compute_node_temporal_embeddings(
                node_memories=updated_node_memories, node_ids=node_ids,
    ...
    def get_updated_memories(self, node_ids: np.ndarray, node_raw_messages: dict):
        ...
        # unique_node_ids, ndarray, shape (num_unique_ncode_ids, ), array of unique node ids
...
class MemoryUpdater(nn.Module):
    ...
    def update_memories(self, unique_node_ids: np.ndarray, unique_node_messages: torch.Tensor,
        ...
        #         torch.from_numpy(unique_node_timestamps).float().to(self.node_last_updated_times.device)).all().item(), "Trying to update memory to time in the past!"
        #
        # assert (self.memory_bank.get_node_last_updated_times(unique_node_ids) <= torch.from_numpy(
        #     unique_node_timestamps).float().to(self.memory_bank.node_last_updated_times.device)).all().item(), "Trying to update memory to time in the past!"
        #
        # print("####", self.memory_bank.get_node_last_updated_times(unique_node_ids).device)
        assert (self.memory_bank.get_node_last_updated_times(unique_node_ids) <= torch.from_numpy(
            unique_node_timestamps).float().to(self.memory_bank.get_node_last_updated_times(
            unique_node_ids).device)).all().item(), "Trying to update memory to time in the past!"
