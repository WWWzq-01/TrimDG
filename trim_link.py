...
if __name__ == "__main__":
    ...
    score_temporal = calculate_temporal_pagerank(np.stack([full_data.src_node_ids, full_data.dst_node_ids]),
    ...
    train_neighbor_sampler = get_neighbor_sampler(data=train_data, sample_neighbor_strategy=args.sample_neighbor_strategy,
                                                 time_scaling_factor=args.time_scaling_factor, seed=0,
                                                 temporal_pagerank=score_temporal, args=args)
    ...
    for run in range(args.num_runs):
        ...
        for epoch in range(args.num_epochs):
            ...
            batch_sample_num = int(args.batch_size * args.batch_rate)
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                # del positive_probabilities, negative_probabilities
                # torch.cuda.empty_cache()
                positive_probabilities = positive_probabilities.detach().cpu()
                negative_probabilities = negative_probabilities.detach().cpu()
                ...
                # loss=loss.detach()
                del loss, predicts, labels
                del batch_src_node_embeddings, batch_dst_node_embeddings, batch_neg_dst_node_embeddings,batch_neg_src_node_embeddings,
                torch.cuda.empty_cache()
                if args.model_name in ['JODIE', 'DyRep', 'TGN']:
