...
def get_link_prediction_args(is_evaluation: bool = False):
    ...
    parser.add_argument('--model_name', type=str, default='JODIE',
    ...
    parser.add_argument('--freeze', type=bool, default=True, help='whether in freeze setting')
...
def get_node_classification_args():
    ...
    # parser.add_argument('--dataset_name', type=str, help='dataset to be used', default='wikipedia', choices=['wikipedia', 'reddit','dgraph'])
    parser.add_argument('--dataset_name', type=str, help='dataset to be used', default='wikipedia',
                        choices=['wikipedia', 'reddit', 'dgraph'])
    parser.add_argument('--batch_size', type=int, default=200, help='batch size')
    ...
    parser.add_argument('--idx', type=int, default=1, help='number of version')
    parser.add_argument('--freeze', action='store_true', default=False, help='enable freeze setting')
    parser.add_argument('--random', action='store_true', default=False, help='random setting')
    parser.add_argument('--num_neighbors', type=int, default=20, help='number of neighbors to sample for each node')
    parser.add_argument('--sample_neighbor_strategy', type=str, default='recent',
                        choices=['uniform', 'recent', 'time_interval_aware'], help='how to sample historical neighbors')
    parser.add_argument('--time_scaling_factor', default=1e-6, type=float,
    ...
    assert args.dataset_name in ['wikipedia', 'reddit', 'dgraph'], f'Wrong value for dataset_name {args.dataset_name}!'
    if args.load_best_configs:
...
def test_args():
    ...
    # parser.add_argument('--model_name', type=str, default='DyGFormer', help='name of the model',
    #                     choices=['JODIE', 'DyRep', 'TGAT', 'TGN', 'CAWN', 'TCL', 'GraphMixer', 'DyGFormer'])
    parser.add_argument('--model_name', type=str, default='TGN', help='name of the model',
                        choices=['JODIE', 'DyRep', 'TGAT', 'TGN', 'CAWN', 'TCL', 'GraphMixer', 'DyGFormer'])
    parser.add_argument('--gpu', type=int, default=1, help='number of gpu to use')
    parser.add_argument('--idx', type=int, default=1, help='number of version')
    parser.add_argument('--freeze', action='store_true', default=False, help='enable freeze setting')
    parser.add_argument('--random', action='store_true', default=True, help='random setting')
    ...
    parser.add_argument('--num_layers', type=int, default=2, help='number of model layers')
    parser.add_argument('--walk_length', type=int, default=1, help='length of each random walk')
    parser.add_argument('--time_gap', type=int, default=20, help='time gap for neighbors to compute node features')
    parser.add_argument('--time_feat_dim', type=int, default=100, help='dimension of the time embedding')
    parser.add_argument('--position_feat_dim', type=int, default=172, help='dimension of the position embedding')
    parser.add_argument('--patch_size', type=int, default=1, help='patch size')
    parser.add_argument('--channel_embedding_dim', type=int, default=50, help='dimension of each channel embedding')
...
def get_method_args():
    ...
    # parser.add_argument('--dataset_name', type=str, help='dataset to be used', default='wikipedia', choices=['wikipedia', 'reddit','dgraph'])
    parser.add_argument('--dataset_name', type=str, help='dataset to be used', default='wikipedia',
                        choices=['wikipedia', 'reddit', 'dgraph', 'mooc', 'BitAlpha', 'BitOtc'])
    parser.add_argument('--batch_size', type=int, default=200, help='batch size')
    # parser.add_argument('--model_name', type=str, default='DyGFormer', help='name of the model',
    #                     choices=['JODIE', 'DyRep', 'TGAT', 'TGN', 'CAWN', 'TCL', 'GraphMixer', 'DyGFormer'])
    parser.add_argument('--model_name', type=str, default='GraphMixer', help='name of the model',
                        choices=['JODIE', 'DyRep', 'TGAT', 'TGN', 'CAWN', 'TCL', 'GraphMixer', 'DyGFormer','FastTGAT'])
    parser.add_argument('--gpu', type=int, default=1, help='number of gpu to use')
    parser.add_argument('--idx', type=int, default=2, help='number of version')
    parser.add_argument('--freeze', action='store_true', default=False, help='enable freeze setting')
    parser.add_argument('--random', action='store_true', default=False, help='random setting')
    parser.add_argument('--num_neighbors', type=int, default=20, help='number of neighbors to sample for each node')
    parser.add_argument('--cache', type=int, default=1, help='Whether we will use cache')
    parser.add_argument('--edge_sampling', type=int, default=1, help='Whether we will adaptive edge sampling ')
    parser.add_argument('--batch_rate', type=float, default=0.5, help='Whether we will sampling the hard sample')
    parser.add_argument('--batch_sampling', type=int, default=1, help='Whether we will use the batch sampling strategy')
    parser.add_argument('--mutual', type=int, default=0, help='Whether we will conduct infomax optimization ')
    parser.add_argument('--bypass', type=float, default=0.01, help='bypass probability ')
    parser.add_argument('--sample_neighbor_strategy', type=str, default='our',
                        choices=['uniform', 'recent', 'time_interval_aware', 'our', 'all_sample'],
                        help='how to sample historical neighbors')
    parser.add_argument('--fast_attention', type=int, default=0, help='Whether we will use fast attention')
    # parser.add_argument('--presampling_rate', type=float, default=0.9, help='presampling_rate')
    parser.add_argument('--presampling_total_rate', type=float, default=0.6, help='total_presampling_rate')
    parser.add_argument('--time_scaling_factor', default=1e-6, type=float,
                        help='the hyperparameter that controls the sampling preference with time interval, '
                             'a large time_scaling_factor tends to sample more on recent links, 0.0 corresponds to uniform sampling, '
    ...
    parser.add_argument('--optimizer', type=str, default='Adam', choices=['SGD', 'Adam', 'RMSprop'],
                        help='name of optimizer')
    parser.add_argument('--gputest', action='store_true', default=False,
                        help='是否启用测试功能（默认：False）')
    parser.add_argument('--weight_decay', type=float, default=0.0, help='weight decay')
    parser.add_argument('--patience', type=int, default=20, help='patience for early stopping')
    parser.add_argument('--val_ratio', type=float, default=0.15, help='ratio of validation set')
    parser.add_argument('--test_ratio', type=float, default=0.15, help='ratio of test set')
    ...
    assert args.dataset_name in ['wikipedia', 'reddit', 'dgraph', 'mooc', 'BitAlpha', 'BitOtc'], f'Wrong value for dataset_name {args.dataset_name}!'
    if args.load_best_configs:
        load_node_classification_best_configs(args=args)
